...
export const Route = createFileRoute("/staff/products")({
  head: () => ({ meta: [{ title: "Products — Staff" }] }),
  component: Inventory,
});
      ...
      <div className="card-soft overflow-hidden">
        <div className="p-4 md:p-5 flex flex-wrap gap-3 items-center border-b border-border/60">
          <div className="relative flex-1 min-w-64">
            ...
            <Input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search model, IMEIâ€¦" className="pl-9 rounded-full bg-secondary border-0" />
          </div>
                ...
                <tr key={p.id} className="border-t border-border/60 hover:bg-surface/40 transition-colors">
                  <td className="px-5 py-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="min-w-0"><div className="font-medium truncate">{p.model}</div><div className="text-xs text-muted-foreground">{p.brand} Â· {p.color}</div></div>
                    </div>
                  </td>
