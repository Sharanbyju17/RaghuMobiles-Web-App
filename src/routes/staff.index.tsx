...
export const Route = createFileRoute("/staff/")({
  head: () => ({ meta: [{ title: "Staff Dashboard — Raghu Mobiles" }] }),
  component: Dashboard,
});
...
function Dashboard() {
  ...
  return (
    <StaffShell
      title="Overview"
      subtitle={`Welcome back, ${session?.user?.fullName || user?.fullName || "Staff"}`}
      actions={<Button asChild className="rounded-full"><Link to="/staff/pos"><ScanLine className="h-4 w-4 mr-1.5" />Start POS</Link></Button>}
    >
